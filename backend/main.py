...
This module wires together the four routers (health, model-info, transaction
scoring, spike analysis) and adds four cross-cutting pieces of
infrastructure: structured request logging, a startup artifact pre-load,
CORS for browser access (Phase 6), and API-key auth (Phase 7). It contains
NO risk-scoring, feature-generation, or spike-detection logic - see
src/risk_engine/ for all of that.

PHASE 6 - CORS: the React dev server (or a deployed frontend) runs on a
different origin than this API, so the browser blocks fetch() calls without
an explicit CORS grant. `settings.cors_allow_origins` (backend/core/config.py,
Phase 7: now env-driven, comma-separated) lists the allowed origins - not a
wildcard. CORS only tells a BROWSER which origins may read this API's
responses; it does nothing to stop a direct, non-browser request (curl,
another server, a script) from any origin - that is what Phase 7's API key
is for, not CORS.

PHASE 7 - API-KEY AUTH (see backend/core/auth.py and
reports/phase7_summary.md for the full design and its explicit limits): a
single shared-secret key, required via the `dependencies=[Depends(require_api_key)]`
on every router below EXCEPT `health` - a health check that itself requires
auth is a common ops footgun (load balancers/uptime monitors/orchestrator
readiness probes need to reach it unauthenticated). This is a proportionate
control for a single-tenant prototype, NOT a full identity system - no user
accounts, no roles, no OAuth, no per-caller audit trail. `allow_credentials`
stays False: an API key sent as a header is not a cookie, so nothing about
CORS credential handling needs to change for it.

Also explicitly out of scope for this phase (and the project as a whole,
per reports/phase7_deployment_checklist.md): rate limiting, response
caching, async task queues, websockets/streaming, TLS termination (handled
by whatever sits in front of this service in a real deployment, not by
this code).
"""
import time
